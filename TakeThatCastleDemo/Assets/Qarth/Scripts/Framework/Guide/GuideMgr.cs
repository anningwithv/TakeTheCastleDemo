...
namespace Qarth
{
    ...
    public class GuideMgr : TMonoSingleton<GuideMgr>
    {
        ...
        /// <returns></returns>
        public bool IsGuiding(int guideID) {
            for (int i = 0; i < m_TrackingGuideList.Count; i++)
            {
                if (m_TrackingGuideList[i].guideID == guideID) {
                    return m_TrackingGuideList[i].IsActive();
                }
            }
