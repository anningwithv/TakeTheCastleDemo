            ...
            if(QUI.BeginFadeGroup(anim.rotateIsExpanded.faded))
            {
                ...
                QUI.BeginVertical(width);
                {
                    ...
                    QUI.BeginHorizontal(width);
                    {
                        ...
                        QUI.Label(QLabel);
                        tempWidth -= QLabel.x;
                        RotateMode rotateMode = anim.rotate.rotateMode;
                        QUI.BeginChangeCheck();
                        rotateMode = (RotateMode)EditorGUILayout.EnumPopup(rotateMode, GUILayout.Width(tempWidth - 20));
                        if (QUI.EndChangeCheck())
                        {
                            Undo.RecordObject(targetObject, "UpdateRotateMode");
                            anim.rotate.rotateMode = rotateMode;
                            QUI.SetDirty(targetObject);
                            AssetDatabase.SaveAssets();
                        }
                    }
