...
namespace DoozyUI
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Shows all the UIElements that have the given name and category.//单独动画
        /// </summary>
        public static void ShowUiElement(string elementName, bool instantActionbool, bool isSingle)
        {
            ShowUiElement(elementName, DUI.UNCATEGORIZED_CATEGORY_NAME, false, isSingle);
        }
        ...
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void ShowUiElement(string elementName, string elementCategory, bool instantAction, bool isSingle)
        {
            if(elementName.Equals(DUI.DEFAULT_ELEMENT_NAME)) { return; }
            Instance.ExecuteShow(elementName, elementCategory, instantAction, isSingle);
        }
        ...
        /// <summary>
        /// This executes the SHOW actions and forces an Instance if required.//单独动画
        /// <para/> This is needed in order to call Show at Start
        /// </summary>
        private void ExecuteShow(string elementName, string elementCategory, bool instantAction, bool isSingle)
        {
            showElementsList = GetUiElements(elementName, elementCategory);
            count = showElementsList != null ? showElementsList.Count : 0;
            ...
            if (count > 0)
            {
                if (showElementsList[showElementsList.Count - 1] != null) //this null check has been added to fix the slim chance that we registered a UIElement to the registry and it has been destroyed/deleted (thus now it's null)
                {
                    showElementsList[showElementsList.Count - 1].gameObject.SetActive(true);

                    if (showElementsList[showElementsList.Count - 1].gameObject.activeInHierarchy)
                    {
                        if (!useOrientationManager)
                        {
                            if (Instance.debugUIElements) { Debug.Log("[UIManager] Showing the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                            showElementsList[showElementsList.Count - 1].Show(instantAction);
                        }
                        else
                        {
                            if (currentOrientation == Orientation.Unknown)
                            ...
                            if (currentOrientation == Orientation.Landscape)
                            {
                                if (showElementsList[showElementsList.Count - 1].LANDSCAPE)
                                {
                                    if (Instance.debugUIElements) { Debug.Log("[UIManager] Showing the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                                    showElementsList[showElementsList.Count - 1].Show(instantAction);
                                }
                                else if (showElementsList[showElementsList.Count - 1].PORTRAIT)
                                {
                                    showElementsList[showElementsList.Count - 1].isVisible = true;
                                    showElementsList[showElementsList.Count - 1].Hide(true);
                                }
                                else
                                {
                                    showElementsList[showElementsList.Count - 1].isVisible = true;
                            ...
                            {
                                if (showElementsList[showElementsList.Count - 1].PORTRAIT)
                                {
                                    if (Instance.debugUIElements) { Debug.Log("[UIManager] Showing the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                                    showElementsList[showElementsList.Count - 1].Show(instantAction);
                                }
                                else if (showElementsList[showElementsList.Count - 1].LANDSCAPE)
                                {
                                    showElementsList[showElementsList.Count - 1].isVisible = true;
                                    showElementsList[showElementsList.Count - 1].Hide(true);
                                }
            ...
            if (count > 0)
            {
                showEffectsList[showEffectsList.Count - 1].gameObject.SetActive(true);
                if (showEffectsList[showEffectsList.Count - 1].gameObject.activeInHierarchy)
                {
                    if (useOrientationManager == false)
                    {
                        showEffectsList[showEffectsList.Count - 1].Show();
                    }
                    else
                    {
                        if (currentOrientation == Orientation.Landscape)
                        {
                            if (showEffectsList[showEffectsList.Count - 1].targetUIElement.LANDSCAPE)
                            {
                                showEffectsList[showEffectsList.Count - 1].Show();
                            }
                            else if (showEffectsList[showEffectsList.Count - 1].targetUIElement.PORTRAIT)
                            {
                                showEffectsList[showEffectsList.Count - 1].isVisible = true;
                                showEffectsList[showEffectsList.Count - 1].Hide();
                            }
                            else
                            {
                                showEffectsList[showEffectsList.Count - 1].isVisible = true;
                        ...
                        {
                            if (showEffectsList[showEffectsList.Count - 1].targetUIElement.PORTRAIT)
                            {
                                showEffectsList[showEffectsList.Count - 1].Show();
                            }
                            else if (showEffectsList[showEffectsList.Count - 1].targetUIElement.LANDSCAPE)
                            {
                                showEffectsList[showEffectsList.Count - 1].isVisible = true;
                                showEffectsList[showEffectsList.Count - 1].Hide();
                            }
        ...
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void HideUiElement(string elementName, bool instantAction)
        {
            HideUiElement(elementName, DUI.UNCATEGORIZED_CATEGORY_NAME, instantAction);
        }

        /// <summary>
        /// Hides all the UIElements that have the given name and the DEFAULT CATEGORY name.//单独动画
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void HideUiElement(string elementName, bool instantAction, bool isSingle)
        {
            HideUiElement(elementName, DUI.UNCATEGORIZED_CATEGORY_NAME, instantAction, isSingle);
        }
        ...
        /// <summary>
        /// Hides all the UIElements that have the given name and category. // 单一动画
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void HideUiElement(string elementName, string elementCategory, bool instantAction,bool isSingle)
        {
            if (elementName.Equals(DUI.DEFAULT_ELEMENT_NAME)) { return; }
            Instance.ExecuteHide(elementName, elementCategory, instantAction, isSingle);
        }
        ...
        /// <summary>
        /// This IEnumerator executes the Hide actions and forces an Instance if required. 是否是单一的动画
        /// </summary>
        private void ExecuteHide(string elementName, string elementCategory, bool instantAction,bool isSingle)
        {
            hideElementsList = GetUiElements(elementName, elementCategory);
            ...
            if (count > 0)
            {
                if (hideElementsList[hideElementsList.Count - 1] != null) //this null check has been added to fix the slim chance that we registered a UIElement to the registry and it has been destroyed/deleted (thus now it's null)
                {
                    if (hideElementsList[hideElementsList.Count - 1].gameObject.activeInHierarchy)
                    {
                        if (Instance.debugUIElements) { Debug.Log("[UIManager] Hiding the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                        hideElementsList[hideElementsList.Count - 1].Hide(instantAction);
                    }
                }
            }
            ...
            if (count > 0)
            {
                if (hideEffectsList[hideEffectsList.Count - 1].gameObject.activeInHierarchy)
                {
                    hideEffectsList[hideEffectsList.Count - 1].Hide();
                }
            }
        }
        #endregion
